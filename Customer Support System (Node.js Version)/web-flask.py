...
def split_into_many(text, max_tokens = max_tokens):
    ...
    for sentence, token in zip(sentences, n_tokens):
        ...
        # If the number of tokens so far plus the number of tokens 
        # in the current sentence is greater than the max number of 
        # tokens, then add the chunk to the list of chunks and reset
        ...
        # Otherwise, add the sentence to the chunk and add 
        # the number of tokens to the total
...
# Note that you may run into rate limit issues depending on how many 
# files you try to embed
...
df['embeddings'] = df.text.apply(lambda x: openai.Embedding.
     create(input=x, engine='text-embedding-ada-002')['data'][0]['embedding'])
