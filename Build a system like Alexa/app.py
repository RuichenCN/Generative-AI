...
def main(model, english, energy, pause, dynamic_energy, wake_word, verbose):
    ...
    threading.Thread(target=record_audio, args=(audio_queue, energy, 
        pause, dynamic_energy,)).start()
...
def reply(result_queue, verbose):
    while True:
        ...
        try:
            ...
            mp3_obj = gTTS(text=choices[np.random.randint(0, 
            len(choices))], lang="en", slow=False)
            if verbose:
                print(e)
        mp3_obj.save("reply.mp3")
        reply_audio = AudioSegment.from_mp3("reply.mp3")
